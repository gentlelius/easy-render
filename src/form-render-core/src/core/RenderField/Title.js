...
const Title = ({ labelClass, labelStyle, schema, displayType }) => {
    ...
    const _displayType = schema.displayType || displayType || globalDisplayType || 'column';
            ...
            {title ? (
                <label
                    className={`fr-label-title ${
                        isCheckBoxType(schema, readOnly) || _displayType === 'column' ? 'no-colon' : ''
                    }`} // checkbox不带冒号
                    title={title}
                >
                    {required === true && <span className="fr-label-required"> *</span>}
                    <span className={`${isObjType ? 'b' : ''} ${_displayType === 'column' ? 'flex-none' : ''}`}>
